...
from GalTransl.SubtitleQuality import find_consecutive_repeat_issue, normalize_subtitle_text
    ...
    async def translate(self, trans_list: CTransList, gptdict=""):
        ...
        while True:  # 一直循环，直到得到数据
            ...
            if degen_flag:
                ...
                repeat_issue = find_consecutive_repeat_issue(result_list, min_repeat=3)
                if repeat_issue is not None:
                    repeated_segment = trans_list[
                        repeat_issue.start_index - 1 : repeat_issue.end_index
                    ]
                    repeated_src = {
                        normalize_subtitle_text(tran.post_jp)
                        for tran in repeated_segment
                        if normalize_subtitle_text(tran.post_jp)
                    }
                    if len(repeated_src) > 1:
                        start_idx = repeated_segment[0].index if repeated_segment else repeat_issue.start_index
                        error_message = (
                            f"-> 第{start_idx}句开始连续"
                            f"{repeat_issue.count}句译文完全相同: {repeat_issue.text}"
                        )
                        error_flag = True
