...
class CGPT4Translate:
    ...
    async def translate(self, trans_list: CTransList, gptdict="", proofread=False):
        ...
        max_service_unavailable_retries = 6
        while True:  # 一直循环，直到得到数据
            try:
                ...
                if any(marker.lower() in str_ex for marker in service_unavailable_markers):
                    ...
                    if service_unavailable_retry_count >= max_service_unavailable_retries:
                        raise RuntimeError(
                            f"连续{service_unavailable_retry_count}次服务不可用，停止在线重试并交给外层回退"
                        ) from ex
                else:
                    service_unavailable_retry_count = 0
                if "quota" in str_ex:
            ...
            repeat_issue = find_consecutive_repeat_issue(
                [
                    trans.post_zh if proofread else trans.pre_zh
                    for trans in result_trans_list
                ],
                min_repeat=3,
            )
            if repeat_issue:
