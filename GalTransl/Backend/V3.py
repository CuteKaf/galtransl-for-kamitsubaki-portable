...
class Chatbot:
    ...
    def _create_client(self, proxy: str, **kwargs) -> httpx.Client:
        kwargs.setdefault("headers", {}).setdefault("User-Agent", "Mozilla/5.0")
        kwargs.setdefault("transport", httpx.HTTPTransport(retries=0))
        if proxy:
            try:
    ...
    def _create_async_client(self, proxy: str, **kwargs) -> httpx.AsyncClient:
        kwargs.setdefault("headers", {}).setdefault("User-Agent", "Mozilla/5.0")
        kwargs.setdefault("transport", httpx.AsyncHTTPTransport(retries=0))
        if proxy:
            try:
    ...
    def _can_use_proxy(self, proxy: str | None) -> bool:
        if not proxy:
            return False
        if proxy.startswith("socks5://") and importlib.util.find_spec("socksio") is None:
            return False
        return True
