...
from GalTransl.SubtitleQuality import format_quality_issue, inspect_srt_quality
...
""")
    ...
    def initSettingsTab(self):
        ...
        for item in os.listdir('whisper-faster'):
            if not item.startswith('faster-whisper'):
                continue
            model_name = item[15:]
            model_dir = faster_whisper_dir / 'models' / model_name
            if (model_dir / 'model.bin').exists():
                faster_whisper_models.append(item)
        whisper_lst = whisper_models + faster_whisper_models + ['不进行听写']
...
class MainWorker(QObject):
    ...
    def _is_local_translator(self, translator: str) -> bool:
        return any(key in translator for key in ('galtransl', 'sakura', 'llamacpp'))

    def _resolve_online_endpoint(self, translator: str, custom_address: str) -> str:
        if translator == 'gpt-custom':
            return (custom_address or 'https://api.openai.com').rstrip('/')
        return ONLINE_TRANSLATOR_MAPPING.get(translator, '').rstrip('/')

    def _should_use_gpt4_backend(self, translator: str, model_name: str) -> bool:
        if self._is_local_translator(translator) or translator == '不进行翻译':
            return False
        model_name = (model_name or '').strip().lower()
        if translator == 'gpt-custom':
            return '3.5' not in model_name
        return False

    def _write_translation_config(self, force_local: bool = False):
        translator = self.master.translator_group.currentText()
        language = self.master.input_lang.currentText()
        gpt_token = self.master.gpt_token.text().strip() or 'sk-XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX'
        gpt_address = self.master.gpt_address.text().strip()
        gpt_model = self.master.gpt_model.text().strip()
        proxy_address = self.master.proxy_address.text().strip()

        with open('project/config.yaml', 'r', encoding='utf-8') as f:
            config_data = yaml.safe_load(f) or {}

        config_data.setdefault('common', {})
        config_data['common']['language'] = f'{language}2zh-cn'

        backend = config_data.setdefault('backendSpecific', {})
        if force_local or self._is_local_translator(translator):
            sakura = backend.setdefault('Sakura', {})
            sakura['endpoint'] = 'http://127.0.0.1:8989'
        else:
            endpoint = self._resolve_online_endpoint(translator, gpt_address)
            if not endpoint:
                endpoint = 'https://api.openai.com'
            for section_name in ('GPT35', 'GPT4'):
                section = backend.setdefault(section_name, {})
                section['tokens'] = [{'token': gpt_token, 'endpoint': ''}]
                section['defaultEndpoint'] = endpoint
                section['rewriteModelName'] = gpt_model

        proxy = config_data.setdefault('proxy', {})
        proxy['enableProxy'] = bool(proxy_address) and not force_local
        proxy['proxies'] = [{'address': proxy_address or 'http://127.0.0.1:7890'}]

        with open('project/config.yaml', 'w', encoding='utf-8') as f:
            yaml.safe_dump(config_data, f, allow_unicode=True, sort_keys=False)

    def _wait_for_sakura_ready(self, expected_model: str, timeout_seconds: int = 120) -> bool:
        start_wait = time()
        while True:
    ...
    def _ensure_sakura_process(self, proc):
        ...
        if not sakura_file:
            raise RuntimeError("未选择Sakura模型文件，无法回退到本地翻译。")

        expected_model = str(Path(sakura_file).name)
        if self._wait_for_sakura_ready(expected_model, timeout_seconds=1):
            self.status.emit("[INFO] 复用现有Sakura翻译器。")
            return proc

        self.status.emit("[INFO] 正在启动Llamacpp翻译器...")
        print(param_llama)
        proc = self._start_process([
            param.replace('$model_file', sakura_file).replace('$num_layers', sakura_mode).replace('$port', '8989')
            for param in param_llama.split()
        ])

        self.status.emit("[INFO] 正在等待Sakura翻译器启动并确认/chat/completions可用...")
        if self._wait_for_sakura_ready(expected_model):
            self.status.emit("[INFO] Sakura翻译器启动并准备就绪！")
            return proc

        self._cleanup_process(proc)
        raise RuntimeError("Sakura翻译器启动超时或模型未加载成功。")

    def _reset_translation_outputs(self):
        for folder in ['project/gt_output', 'project/transl_cache']:
    ...
    def update_translation_config(self):
        ...
        self._write_translation_config(force_local=False)
    ...
    def run(self):
        ...
        if need_translate:
            if self._is_local_translator(translator):
                engine = 'sakura-010'
            else:
                engine = 'gpt4' if self._should_use_gpt4_backend(translator, gpt_model) else 'gpt35-1106'
                fallback_engine = 'sakura-010' if sakura_file else None
        ...
        for idx, input_file in enumerate(input_files):
            ...
            if input_file.endswith('.srt'):
                issue = inspect_srt_quality(Path(input_file))
                if issue:
                    self.status.emit(f"[ERROR] 字幕质量检测失败，已跳过翻译：{format_quality_issue(issue)}")
                    continue
                ...
                generated_srt = Path(wav_file[:-4] + '.srt')
                issue = inspect_srt_quality(generated_srt)
                if issue:
                    self.status.emit(f"[ERROR] 语音识别结果疑似退化，已停止后续翻译：{format_quality_issue(issue)}")
                ...
                make_prompt(str(generated_srt), output_file_path)
            ...
            if need_translate and engine == 'sakura-010':
                proc = self._ensure_sakura_process(proc)
                if self._stop_requested:
                    self._cleanup_process(proc)
                    self.finished.emit()
                    return
            ...
            if need_translate:
                ...
                try:
                    self._write_translation_config(force_local=(engine == 'sakura-010'))
                    cfg = CProjectConfig('project','config.yaml')
                    asyncio.run(run_galtransl(cfg, engine))
                except Exception as e:
                    if not fallback_engine or self._stop_requested:
                        self.status.emit(f"[ERROR] 翻译过程中发生错误: {e}")
                        continue
                    self.status.emit(f"[WARNING] 在线翻译失败，正在回退本地Sakura：{e}")
                    try:
                        proc = self._ensure_sakura_process(proc)
                        self._reset_translation_outputs()
                        self._write_translation_config(force_local=True)
                        cfg = CProjectConfig('project','config.yaml')
                        asyncio.run(run_galtransl(cfg, fallback_engine))
                    except Exception as fallback_error:
                        self.status.emit(f"[ERROR] 在线翻译失败，回退本地也失败：{fallback_error}")
                        continue
